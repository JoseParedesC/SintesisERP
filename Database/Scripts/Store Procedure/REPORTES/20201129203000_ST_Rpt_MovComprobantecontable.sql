...
SET LANGUAGE Spanish
	...
	BEGIN
		SELECT
			I.id_concepto,id_cuenta,ISNULL(P.nombre,C.nombre) nombre,CASE WHEN R.primernombre IS NOT NULL 
                         THEN R.primernombre ELSE '' END + ' ' + CASE WHEN R.segundonombre IS NOT NULL THEN R.segundonombre ELSE '' END + ' ' + CASE WHEN R.primerapellido IS NOT NULL 
                         THEN R.primerapellido ELSE '' END + ' ' + CASE WHEN R.segundoapellido IS NOT NULL THEN R.segundoapellido ELSE '' END AS TERCERO,detalle,IIF(I.valor>0,I.valor,0) debito,IIF(I.valor<0,I.valor*-1,0) credito
		FROM 
			...
			INNER JOIN CNT.TipoTerceros TT ON I.id_tercero=TT.ID
			INNER JOIN CNT.Terceros r ON TT.ID_TERCERO=R.id
		WHERE 
