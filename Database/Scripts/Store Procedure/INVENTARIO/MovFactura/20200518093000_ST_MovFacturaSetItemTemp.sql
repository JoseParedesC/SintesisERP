...
Begin Try
	...
	BEGIN		
		IF (@op = 'D')			
			...
			FROM [MOVFacturaItemsTemp] I CROSS APPLY Dbo.ST_FnPrecioArticulo (id_articulo, precio, @valor, I.pordescuento) A
	...
	BEGIN
		...
		BEGIN
					...
					FROM [dbo].[MOVFacturaItemsTemp]  T CROSS APPLY Dbo.ST_FnPrecioArticulo (id_articulo, precio, @count, T.pordescuento) A
					WHERE T.id = @id;
		...
		BEGIN
			...
			IF (ISNULL(@count, 0) != 0)			
				...
				I.inc = A.inc, I.total = A.total, I.preciodes = A.preciodes, I.id_iva = A.id_iva, I.id_inc = A.id_inc
				FROM [MOVFacturaItemsTemp] I CROSS APPLY Dbo.ST_FnPrecioArticulo (id_articulo, precio, @count, I.pordescuento) A
				WHERE I.id = @id AND I.id_factura = @id_factura
			END
	END
	ELSE IF (@column = 'LOTE')
