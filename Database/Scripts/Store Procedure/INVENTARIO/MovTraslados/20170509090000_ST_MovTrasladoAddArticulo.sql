...
If EXISTS (SELECT 1 FROM dbo.SYSOBJECTS WHERE id = OBJECT_ID(N'[dbo].[ST_MovTrasladoAddArticulo]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[ST_MovTrasladoAddArticulo]
GO


CREATE PROCEDURE [dbo].[ST_MovTrasladoAddArticulo]
	@idToken		[VARCHAR] (255),
    @id_articulo	[BIGINT],    
	@id_bodega		[BIGINT],    
	@id_bodegadest	[BIGINT],    
	@serie			[BIT],
	@series			[VARCHAR] (MAX),
	@lote			[BIT], 
...
AS
/***************************************
*Nombre:		[Dbo].[ST_MovTrasladoAddArticulo]
----------------------------------------
