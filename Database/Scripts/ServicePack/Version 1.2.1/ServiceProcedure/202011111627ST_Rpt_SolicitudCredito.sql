...
--changeset ,JPREDES:2 dbms:mssql runOnChange:true endDelimiter:GO stripComments:false
...
BEGIN TRY
	...
	BEGIN
		...
		DECLARE @idToken VARCHAR(255) = (SELECT TOP 1 id_factura FROM [dbo].[MOVFacturaItemsTemp] T INNER JOIN 
																[dbo].[MOVCotizacionItems] CI ON CI.id_articulo = T.id_articulo INNER JOIN
																[dbo].[MOVCotizacion] C ON C.id = CI.id_Cotizacion INNER JOIN
																[CRE].[Solicitudes] S ON S.id_cotizacion = C.id
										 WHERE C.id = @id_user AND S.id = @id_solicitud);
		...
		FROM [dbo].[MOVFacturaItemsTemp] T INNER JOIN 
			 [dbo].[Productos] P ON P.id = T.id_articulo LEFT JOIN
			 [dbo].[MOVCotizacionItems] CI ON CI.id_articulo = P.id
		WHERE T.id_factura = @idToken
