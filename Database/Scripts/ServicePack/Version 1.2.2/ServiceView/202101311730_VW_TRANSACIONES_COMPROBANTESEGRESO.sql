...
UNION ALL
SELECT R.id id, 
                        CONVERT(VARCHAR(6), R.fecha, 112) ANOMES,  IIF(R.id_centrocosto=0,null,R.Id_centrocosto) CENTROCOSTO, R.id NRODOCUMENTO, CONVERT(VARCHAR(10),I.nrofactura) nrofactura, CONVERT(varchar, R.FECHA, 111) FECHADCTO, I.id_cuenta CUENTA, 
                        R.id_proveedor IDEN_TERCERO, '' CODPRODUCTO, '' PRESENPRODUCTO, '' NOMPRODUCTO, I.valor  VALOR, '' FORMAPAGO,0 BASEIMP,0 PORCEIMP, 0 CANTIDAD, 'CE' TIPODOC, CONCAT('ABONO / PAGO CUOTA N� ',I.nrofactura) DESCRIPCION,Dbo.ST_FnGetIdList('PROCE') ESTADO,I.fechavencimiento FECHAVENCIMIENTO,0 CIERRE 
FROM					CNT.MOVComprobantesEgresos AS R INNER JOIN 
						CNT.MOVComprobantesEgresosItems AS I ON R.id=I.id_comprobante 
WHERE					(R.estado = Dbo.ST_FnGetIdList('PROCE'))
UNION ALL
