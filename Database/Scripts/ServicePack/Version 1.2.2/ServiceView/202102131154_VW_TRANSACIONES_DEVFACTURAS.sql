...
WHERE       DF.estado = dbo.ST_FnGetIdList('PROCE') AND FP.nombre = 'Efectivo' AND dbo.FnVlrDescFP(DF.id_factura,FP.id,(DF.total-DF.valoranticipo-DF.dsctofinanciero)) IS NOT NULL and DF.contabilizado=0 AND DF.iscausacion = 0
...
FROM        [dbo].[MOVDevFactura]   AS DF                                  INNER JOIN	
			...
			[dbo].[CNTCuentas]          AS C  ON C.id         = FP.id_cuenta   INNER JOIN
			[dbo].[ST_Listados]         AS S  ON S.id         = F.estado
WHERE       DF.estado = dbo.ST_FnGetIdList('PROCE') AND FP.nombre != 'Efectivo' AND FP.id_tipo != dbo.ST_FnGetIdList('CARTERA')  AND dbo.FnVlrDescFP(DF.id_factura,FP.id,(DF.total-DF.valoranticipo-DF.dsctofinanciero)) IS NOT NULL  and DF.contabilizado=0 AND DF.iscausacion = 0
GROUP BY  CL.id, DF.fecha, C.id, FP.nombre, F.created, DF.id,f.consecutivo,DF.id_centrocostos,DF.total,DF.id_factura,FP.id,DF.valoranticipo,F.prefijo,DF.dsctofinanciero
UNION
SELECT        DF.id id, CONVERT(VARCHAR(6), DF.fecha, 112) ANOMES,  IIF(DF.id_centrocostos=0,null,DF.Id_centrocostos) CENTROCOSTO, DF.id NRODOCUMENTO, CONCAT(F.prefijo,'-',F.consecutivo) nrofactura, CONVERT(varchar, DF.fecha, 111) FECHADCTO, 
...
FROM        [dbo].[MOVDevFactura]   AS DF				              INNER JOIN	
			...
			[dbo].[ST_Listados]			AS S  ON S.id  = F.estado
WHERE      DF.estado = dbo.ST_FnGetIdList('PROCE')  AND DF.valoranticipo != 0  and DF.contabilizado=0 AND DF.iscausacion = 0
UNION ALL
SELECT      DF.id id, CONVERT(VARCHAR(6), DF.fecha, 112) ANOMES,  IIF(DF.id_centrocostos=0,null,DF.Id_centrocostos) CENTROCOSTO, DF.id NRODOCUMENTO, CONCAT(F.prefijo,'-',F.consecutivo) nrofactura, CONVERT(varchar, DF.fecha, 111) FECHADCTO, 
...
FROM        [dbo].[MOVDevFactura]   AS DF				              INNER JOIN	
			...
			[dbo].[ST_Listados]			AS S  ON S.id  = F.estado
WHERE      DF.estado = dbo.ST_FnGetIdList('PROCE')  AND DF.dsctofinanciero != 0  and DF.contabilizado=0 AND DF.iscausacion = 0
UNION ALL
SELECT      DF.id id, CONVERT(VARCHAR(6), DF.fecha, 112) ANOMES,  IIF(DF.id_centrocostos=0,null,DF.Id_centrocostos) CENTROCOSTO, DF.id NRODOCUMENTO, CONCAT(F.prefijo,'-',F.consecutivo) nrofactura, CONVERT(varchar, DF.fecha, 111) FECHADCTO, 
...
WHERE       DF.estado = dbo.ST_FnGetIdList('PROCE') AND  Fi.iva > 0 AND SL.nombre!='SERVICIO'  and DF.contabilizado=0 AND DF.iscausacion = 0
...
FROM        [dbo].[MOVDevFactura] AS DF                          INNER JOIN	
			...
			[dbo].[ST_Listados]     AS S  ON S.id   = F.estado       INNER JOIN 
			[dbo].[ST_Listados]     AS SL ON SL.id  = C.tipoproducto
WHERE       DF.estado = dbo.ST_FnGetIdList('PROCE') AND  Fi.inc > 0   and DF.contabilizado=0 AND DF.iscausacion = 0
...
FROM        [dbo].[MOVDevFactura] AS DF						                          INNER JOIN
			...
			[dbo].[ST_Listados]     AS S   ON S.id          = F.estado       INNER JOIN
			[dbo].[ST_Listados]     AS SL  ON SL.id         = Z.tipoproducto
WHERE       DF.estado = dbo.ST_FnGetIdList('PROCE') AND SL.nombre != 'SERVICIO' and z.inventario = 1 and DF.contabilizado=0 AND DF.iscausacion = 0
...
FROM CNT.VW_TRANSACCIONES_MOVCAUSACION WHERE FACTURA = 1
