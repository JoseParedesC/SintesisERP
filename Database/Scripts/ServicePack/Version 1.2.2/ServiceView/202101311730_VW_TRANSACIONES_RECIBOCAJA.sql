...
***************************************/  SELECT R.id id, 
                        CONVERT(VARCHAR(6), R.fecha, 112) ANOMES, iIf(R.id_centrocostos=0,null,R.Id_centrocostos) CENTROCOSTO, R.id NRODOCUMENTO, I.id_factura nrofactura, CONVERT(varchar, R.FECHA, 111) FECHADCTO, I.id_cuenta CUENTA, 
                        R.id_cliente IDEN_TERCERO, '' CODPRODUCTO, '' PRESENPRODUCTO, '' NOMPRODUCTO, I.pagoCuota*-1  VALOR, '' FORMAPAGO,0 BASEIMP,0 PORCEIMP  ,0 CANTIDAD, 'RC' TIPODOC, CONCAT('ABONO / PAGO CUOTA N� ',i.cuota) DESCRIPCION,Dbo.ST_FnGetIdList('PROCE') ESTADO,CONVERT(SMALLDATETIME,I.vencimiento_interes,120) FECHAVENCIMIENTO,0 CIERRE 
FROM            CNT.MOVReciboCajas AS R INNER JOIN 
				CNT.MOVReciboCajasItems AS I ON  R.id=I.id_recibo 
WHERE        (R.estado = Dbo.ST_FnGetIdList('PROCE'))  AND I.pagoCuota>0
UNION ALL
SELECT R.id id, 
                         CONVERT(VARCHAR(6), R.fecha, 112) ANOMES, iIf(R.id_centrocostos=0,null,R.Id_centrocostos) CENTROCOSTO, R.id NRODOCUMENTO, I.id_factura nrofactura, CONVERT(varchar, R.FECHA, 111) FECHADCTO, CU.id CUENTA, 
                        R.id_cliente IDEN_TERCERO, '' CODPRODUCTO, '' PRESENPRODUCTO, '' NOMPRODUCTO, I.pagoInteres*-1  VALOR, '' FORMAPAGO,0 BASEIMP,0 PORCEIMP  ,0 CANTIDAD, 'RC' TIPODOC, CONCAT('ABONO / PAGO INTERES N� ',I.cuota) DESCRIPCION,Dbo.ST_FnGetIdList('PROCE') ESTADO,NULL FECHAVENCIMIENTO,0 CIERRE 
  FROM			CNT.MOVReciboCajas AS R INNER JOIN 
				CNT.VW_MOVReciboCajasItems AS I ON R.id=I.id_recibo 
				cross apply (SELECT ID FROM CNTCuentas WHERE codigo=dbo.GETParametrosValor('CUENTAINTERESMORA')) CU
