...
If exists (select 1 from dbo.sysobjects where id = object_id(N'[CNT].[VW_TRANSACIONES_ANTICIPOS]') and OBJECTPROPERTY(id, N'IsView') = 1)
BEGIN
Drop View [CNT].[VW_TRANSACIONES_ANTICIPOS]
END
GO


CREATE VIEW [CNT].[VW_TRANSACIONES_ANTICIPOS]
AS
/***************************************
*Nombre:		[CNT].[VW_TRANSACIONES_ANTICIPOS]
----------------------------------------
*Tipo:			Vista
*creaci�n:		3/06/20
*Desarrollador: (JTOUS)
...
FROM    MOVAnticipos AS E INNER JOIN FormaPagos AS F ON F.id = E.id_formaPago
