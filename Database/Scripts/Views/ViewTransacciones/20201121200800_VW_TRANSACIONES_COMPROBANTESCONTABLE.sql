...
AS
		SELECT        C.id,  CONVERT(VARCHAR(6), C.fecha, 112) ANOMES, C.id_centrocosto AS CENTROCOSTO, C.id AS NRODOCUMENTO, ISNULL(I.factura,'') AS nrofactura, CONVERT(varchar, C.fecha, 111) AS FECHADCTO, U.id AS CUENTA, 
