...
***************************************/ 
SELECT R.id id, 
                        CONVERT(VARCHAR(6), R.fecha, 112) ANOMES,  R.id_centrocosto CENTROCOSTO, R.id NRODOCUMENTO, '' nrofactura, CONVERT(varchar, R.FECHA, 111) FECHADCTO,C.id CUENTA, 
...
UNION ALL
SELECT R.id id, 
                        CONVERT(VARCHAR(6), R.fecha, 112) ANOMES,  R.id_centrocosto CENTROCOSTO, R.id NRODOCUMENTO, CONVERT(VARCHAR(10),S.nrofactura) nrofactura, CONVERT(varchar, R.FECHA, 111) FECHADCTO, S.id_cuenta CUENTA, 
...
WHERE					(R.estado = Dbo.ST_FnGetIdList('PROCE'))
UNION ALL
SELECT					E.id id, CONVERT(VARCHAR(6),E.fecha, 112) ANOMES, E.id_centrocosto CENTROCOSTO, E.id NRODOCUMENTO, '' nrofactura, CONVERT(varchar, E.FECHA, 111) FECHADCTO, 
