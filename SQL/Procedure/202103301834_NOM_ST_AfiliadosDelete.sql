...
If EXISTS (SELECT 1 FROM dbo.SYSOBJECTS WHERE id = OBJECT_ID(N'[NOM].[ST_AfiliadosDelete]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [NOM].[ST_AfiliadosDelete]
GO
CREATE PROCEDURE [NOM].[ST_AfiliadosDelete]
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 FROM [NOM].[Afiliados] WHERE id = @id )
	BEGIN
