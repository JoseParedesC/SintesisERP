...
If EXISTS (SELECT 1 FROM dbo.SYSOBJECTS WHERE id = OBJECT_ID(N'[NOM].[ST_PrimasDelete]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [NOM].[ST_PrimasDelete]
GO
CREATE PROCEDURE [NOM].[ST_PrimasDelete]
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 FROM [NOM].[Primas] WHERE id = @id )
	BEGIN
