...
If EXISTS (SELECT 1 FROM dbo.SYSOBJECTS WHERE id = OBJECT_ID(N'[NOM].[ST_HorarioDelete]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [NOM].[ST_HorarioDelete]
GO
CREATE PROCEDURE [NOM].[ST_HorarioDelete]
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 FROM [NOM].[Horario] WHERE id = @id )
	BEGIN
