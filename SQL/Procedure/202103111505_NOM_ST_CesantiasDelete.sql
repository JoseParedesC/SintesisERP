...
If EXISTS (SELECT 1 FROM dbo.SYSOBJECTS WHERE id = OBJECT_ID(N'[NOM].[ST_CesantiasDelete]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [NOM].[ST_CesantiasDelete]
GO
CREATE PROCEDURE [NOM].[ST_CesantiasDelete]
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 FROM [NOM].[Cesantias] WHERE id = @id )
	BEGIN
