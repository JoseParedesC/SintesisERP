...
CREATE PROCEDURE [NOM].[ST_ParamsAnualSave]
	...
	@porcen_cajacompensacion NUMERIC(4,2),
	@num_max_seguridasocial BIGINT,
	@id_cuentacobrar BIGINT,
	@id_cuenta_arl BIGINT,
...
BEGIN TRY
	...
	BEGIN
		...
		INSERT INTO [NOM].[ParamsAnual_Empleador] (porcen_salud, porcen_pension, num_salariosMinSalud, id_usercreated, id_userupdated)
			SELECT @porcen_salud_empleador, @porcen_pension_empleador , @num_salmin_salud_empleador, @id_user, @id_user
		...
		INSERT INTO [NOM].[ParamsAnual_Empleado] (porcen_salud, porcen_pension,num_salariosMinICBF, num_salariosMinSENA, porcen_icbf, porcen_sena, num_salariosMinSegSocial, id_usercreated, id_userupdated)
			SELECT @porcen_salud_empleado, @porcen_pension_empleado, @num_salmin_icbf, @num_salmin_sena, @porcen_icbf, @porcen_sena, @num_max_seguridasocial, @id_user, @id_user

		SET @id_empleado = SCOPE_IDENTITY()


		INSERT INTO [NOM].[ParamsAnual] (salario_MinimoLegal, salario_Integral, aux_transporte, id_interesCesantias, uvt, porcen_saludTotal, porcen_pencionTotal, porcen_cajacompensacion, id_cuentacobrar, id_cuentaarl, id_horasExt, id_parametrosEmpleado, id_parametrosEmpleador,exonerado , id_usercreated, id_userupdated, fecha_vigencia)
			SELECT @salarioMinimoLegal, @salIntegral, @auxTrans, @int_Ces, @uvt, @porcen_salud_total , @porcen_pension_total, @porcen_cajacompensacion, @id_cuentacobrar, @id_cuenta_arl, @id_horas, @id_empleado, @id_empleador, @exonerado, @id_user, @id_user, @fecha_vigencia

		SET @id_RETURN = SCOPE_IDENTITY()


		INSERT INTO [NOM].[ParamsAnual_Solid] (desde, hasta, porcentaje, id_usercreated, id_userupdated, id_parametros)
			SELECT desde, hasta, porcen , @id_user, @id_user, @id_RETURN FROM @tbl_solid
	...
	BEGIN
		...
		INSERT INTO [NOM].[ParamsAnual_Solid] (desde, hasta, porcentaje, id_usercreated, id_userupdated, id_parametros)
			SELECT desde, hasta, porcen , @id_user, @id_user, @id FROM @tbl_solid
		...
		UPDATE [NOM].[ParamsAnual_Empleador] 
			SET porcen_salud = @porcen_salud_empleador , 
				porcen_pension = @porcen_pension_empleador , 
		...
		UPDATE [NOM].[ParamsAnual_Empleado] 
			SET porcen_salud = @porcen_salud_empleado , 
			porcen_pension = @porcen_pension_empleado ,
			num_salariosMinICBF = @num_salmin_icbf, 
			num_salariosMinSENA = @num_salmin_sena, 
			porcen_icbf = @porcen_icbf , 
			porcen_sena = @porcen_sena , 
		...
		UPDATE [NOM].[ParamsAnual] 
			...
			exonerado = @exonerado,
			porcen_saludTotal = @porcen_salud_total ,
			porcen_pencionTotal = @porcen_pension_total ,
			porcen_cajacompensacion = @porcen_cajacompensacion, 
			id_cuentacobrar = @id_cuentacobrar,
			id_cuentaarl = @id_cuenta_arl,
