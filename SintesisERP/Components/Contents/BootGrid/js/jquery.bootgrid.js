...
; (function ($, window, undefined) {
    ...
    function getRequest() {
        var column = Object.keys(this.sortDictionary)[0];
        var request = {
            page: this.current,
            quantity: this.rowCount,
            sort: this.sortDictionary[column],
            column: column,
    ...
    function init() {
        ...
        if ((this.options.ajax && !this.options.autoColumn) || !this.options.ajax) {
        ...
        firstHeadRow.children().each(function () {
            var $this = $(this),
                ...
                column = {
                    ...
                    text: (data.columntext == 'html'? $this.html():$this.text()),
    ...
    */
    ...
    function loadData() {
        ...
        if (this.options.ajax) {
                    ...
                    if (response.err) {
                        ...
                        if (that.options.autoColumn) {
                            ...
                            if (data.length > 0) {
                                ...
                                var thead = $('<thead/>');
                                var tr = $('<tr/>');
                                if (that.options.selectSearch) {
                                    var tdedit = $('<td/>').attr({ 'data-column-id': "edit", 'data-formatter': "edit", 'data-sortable': "false" }).html('Seleccionar');
                                    tr.append(tdedit);
                                }
                                var allcolumn = (that.options.fields.length > 0) ? that.options.fields : Object.keys($row);
                                $.each(allcolumn, function (i, e) {
                                    var td = $('<th />').attr('data-column-id', e).html(e.charAt(0).toUpperCase() + e.slice(1));
                                    tr.append(td);
                                });
                                thead.append(tr);
                                var elemento = that.element;
                                //elemento.find('thead').remove();
                                elemento.find('tbody').empty();
                                elemento.prepend(thead);
                                that.options.autoColumn = false;
                                loadColumns.call(that);
            ...
            window.setTimeout(function () { update(rows, total); }, 3);
        }
    }
    ...
    function registerRowEvents(tbody) {
       ...
       /* tbody.off("click" + namespace, "> tr")
            .on("click" + namespace, "> tr", function (e) {
                ...
                if ($this.find('td.no-results').length > 0)
                    return false;

                var id = (that.identifier == null) ? $this.data("row-id") :
    ...
    Grid.defaults = {
        ...
        rowCount: [20, 50, 100, 200], // rows per page int or array of int (-1 represents "All")
        autoColumn: false,
        fields: [],
        selectSearch : false,
        /**
        ...
        css: {
            ...
            right: "text-right",
            search: "search form-group col-lg-6 col-md-6 col-sm-8 col-xs-12 sn_padding", // must be a unique class name or constellation of class names within the header and footer
        ...
        labels: {
            all: "Todos",
            infos: "Registos del {{ctx.start}} al {{ctx.end}} de {{ctx.total}} ",
            loading: "Cargando...",
        ...
        templates: {
            ...
            header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-12 actionBar sn_padding\"><p class=\"{{css.search}}\"></p><p class=\"{{css.actions}}\"></p></div></div></div>",
