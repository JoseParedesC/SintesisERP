var JsonValidate = [
    ...
    { id: 'ds_cliente', type: 'TEXT', htmltype: 'INPUT', required: false, depends: true, iddepends: 'esFE' }];
...
$(document).ready(function () {
    ...
    $('#v_code').autocomplete({
        ...
        onSearchStart: function (query) {
            query.params = JSON.stringify({ filtro: query.keyword, op: 'A', o: '', formula: 0, id_prod: 0, mod: 'P' });
        },
        minChars: 2,
...
$(document).ready(function () {
    ...
    $('#btnFact').click(function () {
        if (validate(JsonValidate)) {
            ...
            sJon.id_tercero = ($('#cd_cliente').val() == '') ? "0" : $('#cd_cliente').val();
            sJon.isFE = $('#esFE').prop('checked');
            sJon.id_vendedor = ($('#cd_vendedor').val() == '') ? "0" : $('#cd_vendedor').val();
            ...
            if (cambio >= 0) {
                ...
                MethodService("Facturas", "FacturarFacturaPos", JSON.stringify(sJon), "CallbackComodity");
...
function cleanpagocred() {
    ...
    $('#nrocuotas2').val('0.00');
    $('#nrodias2').val('1');
    $('#id_tipoven,#tipo_cartera').val('').selectpicker('refresh');
