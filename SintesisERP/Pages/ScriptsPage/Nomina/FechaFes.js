/* Se valida si las casillas del formulario
    están vacios */
var JsonValidate = [
    { id: 'dt_fecha', type: 'DATE', htmltype: 'INPUT', required: true, depends: false, iddepends: '' }
];

window.gridfecha;
/*carga la tabla mostrada en FechaFes.aspx 
utilisando la función c# FechaFesList*/
function Loadtable() {
    window.gridfecha = $("#tblfecha").bootgrid({
        ajax: true,
        post: function () {
            return {
                'params': "",
         ...
         * FechaFes en sus respectivas funciónes para 'editar' y 'eliminar' respectivamente */
        window.gridfecha.find(".command-edit").on("click", function (e) {
            removeerror();
            id = $(this).data("row-id")
            params = {};
            params.id = id;
            MethodService("FechaFes", "FechaFesGet", JSON.stringify(params), 'EndCallbackGet');
        }).end().find(".command-delete").on("click", function (e) {
            if (confirm("Desea eliminar esta fecha?")) {
                id = $(this).data("row-id");
                params = {};
                params.id = id;
                MethodService("FechaFes", "FechaFesDelete", JSON.stringify(params), 'EndCallbackupdate');
            }
        });
...
/*función para el boton circular diseñado en FechaFes.aspx
...
y enviarlo a la funcion FechaFesSave*/
$('#btnSave').click(function (e) {
    if (validate(JsonValidate)) {
        params = {};
        params.id = $(this).attr('data-id');
        params.fecha = $('#dt_fecha').val();
        var btn = $(this);
        btn.button('loading');
        MethodService("FechaFes", "FechaFesSave", JSON.stringify(params), "EndCallbackArticle");
    }
});
...
/*recarga la tabla FechaFes.aspx*/
