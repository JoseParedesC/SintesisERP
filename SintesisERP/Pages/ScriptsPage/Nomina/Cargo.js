...
function formReset() {
    ...
    $('#descripcion').val('');
    $('#funciones').text('');
    document.getElementById('ch_funcesp').checked = false;
    $('#idcargo').val(0);
}
...
$('#btnSave').click(function (e) {
    if (validate(JsonValidate)) {
        ...
        params.fungenerales = $('#funciones')[0].innerText;
        params.funespecifica = document.getElementById('ch_funcesp').checked;
...
function EndCallbackGet(params, answer) {
    if (!answer.Error) {
        ...
        document.getElementById('ch_funcesp').checked = data.funciones_esp
