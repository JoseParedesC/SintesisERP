var JsonValidate = [{ id: 'cd_tipodoc', type: 'TEXT', htmltype: 'SELECT', required: true, depends: false, iddepends: '' },
    ...
    { id: 'm_valorCliente', type: 'TEXT', htmltype: 'INPUT', required: false, depends: false, iddepends: '' }];
    ...
    window.gridbod = $("#tblcommodity").bootgrid({
        ...
        formatters: {
            ...
            "state": function (column, row) {
                return "<a class=\"action command-state\"   data-row-id=\"" + row.id + "\" data-row-factura=\"" + row.factura + "\"   data-row-vlr=\"" + row.valorcuota + "\" data-row-saldoc=\"" + row.saldocuota + "\"   data-row-dias=\"" + row.diasvencido + "\" data-row-interesabonado=\"" + row.interesabono + "\" data-row-id_cuenta=\"" + row.id_cuenta + "\"  >" +
            ...
            "interes": function (column, row) {
                if (row.fechavencimiento.length != 0)
                    return "<span id='interes" + row.id + "' class='tdedit action command-edit' data-type='numeric' data-column='interes' data-min='0' data-max='999999999999999999' data-simbol='%' data-value='" + row.porinteres + "' data-id='" + row.id + "' data-row-vlr='" + row.saldocuota + "' data-row-dias='" + row.diasvencido + "' data-row-interesabonado=\"" + row.interesabono + "\"> % " + row.porinteres.Money() + "</span>";
                else
                    return "<span id='interes" + row.id + "' class='tdedit action command-edit' data-type='numeric' data-column='interes' data-min='0' data-max='999999999999999999' data-simbol='%' data-value='0' data-id='" + row.id + "' data-row-vlr='" + row.saldocuota + "' data-row-dias='" + row.diasvencido + "' data-row-interesabonado=\"" + row.interesabono + "\">% 0.00 </span>";
            },
            "pay": function (column, row) {
                return "<span id='pay" + row.id + "' class='tdedit action command-edit' data-type='numeric' data-column='pay' data-min='0' data-max='999999999999999999' data-simbol='$' data-value='" + row.saldocuota + "' data-id='" + row.id + "'data-dias='" + row.diasvencido + "'>$ 0.00</span>";
            },
            ...
            "Tinteres": function (column, row) {
                return "<span id='Tinteres" + row.id + "' class='tdedit action command-edit' data-type='numeric' data-column='Tinteres' data-min='0' data-max='999999999999999999' data-simbol='$' data-value='" + row.valorcuota + "' data-id='" + row.id + " data-row-interesabonado=\"" + row.interesabono + "\"'>$ 0.00 </span>";
                // return "<input id='tinteres" + row.id + "' data-row-id=\'" + row.id + "'\  data-row-vlr=\"" + row.valorcuota + "\"'  type='text' class='number form-control addart pay '  value='0.00' placeholder=' ' money='true' data-a-dec='.' data-a-sep=', ' data-m-dec='2' data-v-min='0' data-a-sign='$' disabled='disabled' style='border:0; text-align: center' />";
            },
            "Tinteresabono": function (column, row) {
                ...
                // return "<input id='tinteresabono" + row.id + "' data-row-id=\'" + row.id + "'\  data-row-vlr=\"" + row.interesabono + "\"'  type='text' class='number form-control addart pay '  value=\"" + row.interesabono.Money() + "\"' placeholder=' ' money='true' data-a-dec='.' data-a-sep=', ' data-m-dec='2' data-v-min='0' data-a-sign='$' disabled='disabled' style='border:0; text-align: center' />";
                else
                ...
                //      return "<input id='tinteresabono" + row.id + "' data-row-id=\'" + row.id + "'\  data-row-vlr=\"" + row.interesabono + "\"'  type='text' class='number form-control addart pay '  value='0.00' placeholder=' ' money='true' data-a-dec='.' data-a-sep=', ' data-m-dec='2' data-v-min='0' data-a-sign='$' disabled='disabled' style='border:0; text-align: center' />";
            },
            "saldointeres": function (column, row) {
                ...
                //return "<input id='saldointeres" + row.id + "' data-row-id=\'" + row.id + "'\  data-row-vlr=\"" + row.valorcuota + "\"'  type='text' class='number form-control addart pay '  value='0.00' placeholder=' ' money='true' data-a-dec='.' data-a-sep=', ' data-m-dec='2' data-v-min='0' data-a-sign='$' disabled='disabled'  style='border:0; text-align: center' />";
            },
            "cuotas": function (column, row) {
                return "<span  class='anchor' >" + row.cuota_pagar + "</span>";
                // return "<input id='tinteres" + row.id + "' data-row-id=\'" + row.id + "'\  data-row-vlr=\"" + row.valorcuota + "\"'  type='text' class='number form-control addart pay '  value='0.00' placeholder=' ' money='true' data-a-dec='.' data-a-sep=', ' data-m-dec='2' data-v-min='0' data-a-sign='$' disabled='disabled' style='border:0; text-align: center' />";
            },
        }
        ...
        window.gridbod.find(".command-state").on("click", function (e) {
            ...
            interes = Math.round(saldocuota * (((porceinteres * dias) / 100)));// calculamos el valos a pagar en interes teniendo en cuenta la cantidad de dias vencidos
            interesabonado = $(this).data("row-interesabonado");
            $('#saldointeres' + data.rowId).html('$ ' + (interes - interesabonado).Money());
            totalInteres = SetNumber($('#saldointeres' + data.rowId).html());
            totalapagar = parseFloat(SetNumber(totalInteres)) + saldocuota;
            ...
            if (state.attr('class') == 'fa fa-2x fa-fw fa-square-o text-danger  iconfa') {
                ...
                $('#ds_conceptoDscto').val() != '' ? $('#m_valorCliente').removeAttr('disabled') : $('#m_valorCliente').attr('disabled', 'disabled');
            } else {
                ...
                $('#m_valorCliente').attr('disabled', 'disabled')
            }
                ...
                window.gridpagosreal.find(".command-rever").on("click", function (e) {
                    ...
                    if (id != '0' && id != '') {
                        if (confirm("Desea revertir el pago?")) {
                            var sJon = {};
                            sJon.id = id;
                            MethodService("ReciboCaja", "RevertirReciboCaja", JSON.stringify(sJon), "CallbackReversionList")
                        }
                    }
                    ...
                    var idrecibo = id
                    param = 'id|' + idrecibo + ';'
                    PrintDocument(param, 'MOVRECIBO', 'CODE');

                }).end().find(".command-edit").on('click', function (e) {
                    id = $(this).data("row-id");
                    params = {};
                    params.id = id;
                    MethodService("ReciboCaja", "ReciboCajaGet", JSON.stringify(params), 'EndCallbackGet');
...
//Funcion de que carga los conceptos pagados en el recibo de caja
        ...
        post: function () {
            return {
                'params': function () {
                    ...
                    param.fecha = SetDate($('#Text_Fecha').val());
                    param.all = false;//parametro que me indica que me listara solo las cuotas pendientes por pagar al colocar valor 0
                    return JSON.stringify(param);
                },
                'class': 'ReciboCaja',
...
// Evento que actualiza la grilla con la fecha nueva
...
$('#id_conceptoDscto').change(function () {
    window.gridbod.bootgrid("getTotalRowCount") > 0 && SetNumber($('#mtotal_cliente').text()) > 0 ? $('#m_valorCliente').removeAttr('disabled')  : $('#m_valorCliente').attr('disabled', true);
    if ($(this).val().length == 0) 
        $('#m_valorCliente').attr('disabled', true);
});


$('#isReca').on('ifChanged', function () {
    ($(this).prop('checked') ? $('#liuno').css('display', 'inline-block') && window.gridbod.bootgrid('reload') && $('#tab-2').removeClass('active') && $('#tab-1').addClass('active')
        && ($('#tabuno').parent().addClass('active')) && ($('#tabdos').parent().removeClass('active')) :
        $('#liuno').css('display', 'none') && $('#tab-1').removeClass('active') && $('#tab-2').addClass('active') && ($('#tabdos').parent().addClass('active')) && ($('#tabuno').parent().removeClass('active')));
});
...
$('#m_valorCliente').blur(function () {
    ...
    value = isNaN(parseFloat(SetNumber($(this).val()))) ? 0 : parseFloat(SetNumber($(this).val()));
        ...
        if (valor > 0) {
            ...
            if (count == undefined || count.length == 0) {
                ...
                a = $('<a/>').html('<span class="fa fa-2x fa-trash-o text-danger iconfa"></span>').click(function () {
                    ...
                    $('#mtotal').text('$ ' + (parseFloat(SetNumber($('#mtotal_cliente').text())) - parseFloat(SetNumber($('#mtotaldesct_cliente').text()))+ total).Money());
                })
...
$('#btnFact').click(function () {
    ...
    window.gridbod.find(".command-state").each(function () {
        ...
        if (state.attr('class') == 'fa fa-2x fa-fw fa-check-square-o text-success  iconfa') {
            if (SetNumber($('#pay' + $(this).data("row-id")).html()) > 0) {
                pagosXML += '<pago id_factura="' + $(this).data("row-factura") + '" valor="' + SetNumber(($('#pay' + $(this).data("row-id")).html())) + '" porceInteres="' + SetNumber(($('#interes' + $(this).data("row-id")).html())) + '" interes="' + SetNumber(($('#saldointeres' + $(this).data("row-id")).html())) + '" dias="' + $(this).data("row-dias") + '" cuenta="' + $(this).data("row-id_cuenta") + '"/>'
            } else {
                facturar = false;
    ...
    if (cambio >= 0) {
        ...
        if (facturar) {
            ...
            MethodService("ReciboCaja", "ReciboCajaSave", JSON.stringify(params), "EndCallbackPago");
        }
...
//Evento de reversion de recibo de caja
$('#btnRev').click(function () {
    if ($('#id_recibo').val() != '0' && $('#id_recibo').val().trim() != '') {
        if (confirm("Desea revertir el pago?")) {
            var sJon = {};
            sJon.id = ($('#id_recibo').val().trim() == "") ? "0" : $('#id_recibo').val();
            MethodService("ReciboCaja", "RevertirReciboCaja", JSON.stringify(sJon), "CallbackReversion");
        }
    }
});
//Evento de Impresion de documento generado a Recibo de Caja
...
//Funcion de reseteo para nuevo recibo de caja
function newpay() {
    $('#ds_cliente').removeAttr('disabled');
    $('#cd_formapago,.inputsearch,#isReca').removeAttr('disabled');
    var div = $('.diventrada');
    $('#cd_cliente,#id_recibo,#isDescuento,#id_ccostos,#id_conceptoDscto').val(0);
    $('#ds_cliente,#codigoccostos,#ds_conceptoDscto').val('');
    $('#m_valor,#m_valorCliente').val('$ 0.00');
    $('#m_valor,#m_valorCliente').attr('disabled', true);
    $('#mtotal,#mtotal_cliente,#mtotal_concepto,#mtotaldesct_cliente').text('$ 0.00');
    $('.pagonumber').html(0);
    ...
    $('#btnSave,#m_valor,.btnsearch,#Text_Fecha,#cd_formapago,#btnAdd').removeAttr('disabled');
...
function EndCallbackPago(params, answer) {
    if (!answer.Error) {
        ...
        $('#btnSave,#m_valor,.btnsearch,#Text_Fecha,#cd_formapago,#btnAdd,.inputsearch,#isReca').attr('disabled', 'disabled');
        var div = $('.divpagos');
        div.find('select').selectpicker('refresh');
...
//Funcion CallBack que se activa despues de revertir un recibo de caja
...
function CallbackReversionList(Parameter, Result) {
    ...
    if (!Result.Error) {
        window.gridpagosreal.bootgrid('reload');
        toastr.success('Recibo de Caja Revertido.', 'Sintesis ERP');
    }
    else
        toastr.error(Result.Message, 'Sintesis ERP');
}

//Funcion CallBack que se activa despues de Seleccionar un recibo de caja
function EndCallbackGet(params, answer) {
    if (!answer.Error) {
        ...
        $('#btnSave,#m_valor,.btnsearch,#Text_Fecha,#cd_formapago').attr('disabled', 'disabled');
        $('#btnRev').removeAttr('disabled');
