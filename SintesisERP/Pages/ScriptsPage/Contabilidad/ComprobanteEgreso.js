    ...
    window.gridbod = $("#tblcommodity").bootgrid({
        ...
        formatters: {
            ...
            "state": function (column, row) {
                return "<a class=\"action command-state\"   data-row-id=\"" + row.id + "\"  data-row-factura=\"" + row.numfactura + "\" data-row-vlr=\"" + row.SaldoActual + "\" data-row-cuenta=\"" + row.id_cuenta + "\" data-row-fechavence=\"" + row.fechavence + "\"   >" +
...
$('#btnFact').click(function () {
    ...
    window.gridbod.find(".command-state").each(function () {
        ...
        if (state.attr('class') == 'fa fa-2x fa-fw fa-check-square-o text-success  iconfa') {
            pagosXML += '<pago identrada="' + $(this).data("row-id") + '" valor="' + SetNumber(($('#pay' + $(this).data("row-id")).html())) + '" factura="' + $(this).data("row-factura") + '" cuenta="' + $(this).data("row-cuenta") + '" fechavencimiento="' + $(this).data("row-fechavence") + '"/>'
