...
<xsl:stylesheet version="1.5" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl">
  ...
  <xsl:template match="/">    
    <Invoice xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:xades="http://uri.etsi.org/01903/v1.3.2#" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" xmlns:xades141="http://uri.etsi.org/01903/v1.4.1#" xmlns:sts="dian:gov:co:facturaelectronica:Structures-2-1" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2     http://docs.oasis-open.org/ubl/os-UBL-2.1/xsd/maindoc/UBL-Invoice-2.1.xsd" xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2">      
      <ext:UBLExtensions>
        <ext:UBLExtension>
          <ext:ExtensionContent>
            <sts:DianExtensions>
              <sts:InvoiceControl>
                <sts:InvoiceAuthorization><xsl:value-of select="/top/resolucion/codigo"/></sts:InvoiceAuthorization>
                <sts:AuthorizationPeriod>
                  <cbc:StartDate><xsl:value-of select="/top/resolucion/fechaini"/></cbc:StartDate>
                  <cbc:EndDate><xsl:value-of select="/top/resolucion/fechafin"/></cbc:EndDate>
                </sts:AuthorizationPeriod>
                <sts:AuthorizedInvoices>
                  <sts:Prefix><xsl:value-of select="/top/resolucion/prefijo"/></sts:Prefix>
                  <sts:From><xsl:value-of select="/top/resolucion/conini"/></sts:From>
                  <sts:To><xsl:value-of select="/top/resolucion/confin"/></sts:To>
                </sts:AuthorizedInvoices>
              </sts:InvoiceControl>
              <sts:InvoiceSource>
      ...
      <cbc:UUID schemeID="{/top/Empresa/tipoamb}" schemeName="CUFE-SHA384"><xsl:value-of select="/top/cufe"/></cbc:UUID>
      ...
      <cbc:DueDate><xsl:value-of select="/top/fechaact"/></cbc:DueDate>
      <cbc:InvoiceTypeCode>01</cbc:InvoiceTypeCode>
      <cbc:Note>Factura relacionada con: &#160;<xsl:value-of select="/top/factura/numdocumento"/></cbc:Note>
      ...
      <cac:AccountingSupplierParty>
        ...
        <cac:Party>
          <cac:PartyName>
            <cbc:Name><xsl:value-of select="/top/Empresa/nombrecomercial"/></cbc:Name>
          </cac:PartyName>
      ...
      <cac:AccountingCustomerParty>
        <cbc:AdditionalAccountID schemeAgencyID="195"><xsl:value-of select="/top/cliente/TipoTerceroC"/></cbc:AdditionalAccountID>
        <cac:Party>
          <xsl:if test="/top/cliente/TipoTerceroC = 2">
            <cac:PartyIdentification>
              <cbc:ID schemeName="{/top/cliente/tipoid}" schemeID="{/top/cliente/digic}"><xsl:value-of select="/top/cliente/iden"/></cbc:ID>
            </cac:PartyIdentification>
          </xsl:if>
          <cac:PartyName>
            <cbc:Name><xsl:value-of select="/top/cliente/firstname"/> &#160; <xsl:value-of select="/top/cliente/lastname"/></cbc:Name>
          </cac:PartyName>         
          <cac:PhysicalLocation>
            <cac:Address>
      ...
      <xsl:for-each select="/top/items/item">
        <cac:InvoiceLine>
          ...
          <cbc:InvoicedQuantity><xsl:value-of select="cant"/></cbc:InvoicedQuantity>
          ...
          <xsl:if test="dctoA != 0">
            <cac:AllowanceCharge>
              ...
              <cbc:MultiplierFactorNumeric><xsl:value-of select="pordcto"/></cbc:MultiplierFactorNumeric>
              <cbc:Amount currencyID="COP"><xsl:value-of select="totaldcto"/></cbc:Amount>
              <cbc:BaseAmount currencyID="COP"><xsl:value-of select="valor"/></cbc:BaseAmount>
            </cac:AllowanceCharge>
          </xsl:if>
          <cac:TaxTotal>
            <cbc:TaxAmount currencyID="COP"><xsl:value-of select="iva"/></cbc:TaxAmount>
