...
function Loadtable() {
    window.gridmarca = $("#tblmarcas").bootgrid({ //  
        ajax: true,
        post: function () {
            return {
                'params': "",
        ...
        window.gridmarca.find(".command-edit").on("click", function (e) {
            ...
            params = {};
            params.id = id;
            MethodService("Marcas", "MarcasGet", JSON.stringify(params), 'EndCallbackGet');
        }).end().find(".command-delete").on("click", function (e) {
            if (confirm("Desea eliminar la Marca?")) {
                id = $(this).data("row-id");
                params = {};
                params.id = id;
                MethodService("Marcas", "MarcasDelete", JSON.stringify(params), 'EndCallbackupdate');
            }
        }).end().find(".command-state").on("click", function (e) {
            id = $(this).data("row-id");
            params = {};
            params.id = id;
            MethodService("Marcas", "MarcasState", JSON.stringify(params), 'EndCallbackupdate');
        });
    });
}
...
$('#btnSave').click(function (e) {
    if (validate(JsonValidate)) {// valida si los campos cumplen con las condiciones estipuladas en el Json creado a principio de JS
        ...
        var btn = $(this);
        btn.button('loading');
        MethodService("Marcas", "MarcasSave", JSON.stringify(params), "EndCallbackMarca");
    }
});
...
function EndCallbackGet(params, answer) {
    if (!answer.Error) {
        ...
        $('#ModalMarcas').modal({ backdrop: 'static', keyboard: false }, 'show');  // muestra el modal
    }
    else {
