...
var JsonValidate = [
    ...
    { id: 'TipEmpleo', type: 'TEXT', htmltype: 'SELECT', required: true, depends: false, iddepends: '' },
...
$(document).ready(function () {
    ...
    $('#addref').click(function () {
        if (validate(JsonValidateRef)) {
            ...
            td2 = $('<td/>').html(dir);
            td3 = $('<td/>').html(tel);
            td4 = $('<td/>').html(texttipo);
            td5 = $('<td/>').html(textparente);
            td6 = $('<td/>').html();
...
function ActiveDactilar() {


    if (window.iubi != undefined) {
        window.iubi.onclose = () => {
            console.log("dactilar closed");
        };
    }
    ...
    iubi.onerror = function (error) {
        toastr.warning('el servicio del lector u are u 4500 digital persona esta inactivo', 'sintesis creditos');
    };
    console.log(iubi);
    console.log(window.iubi);
    ...
    iubi.onmessage = (function (evt) {
        ...
        switch (data.type) {
            case 'validate':
                ...
                $('#huella').attr('src', 'data:image/png;base64,' + r.data);
        }
    })();
...
function GuardarSolicitud() {
    if (validate(JsonValidateSave)) {
        ...
        //}
        //data.append("folder", 'Creditos/Perfiles');
        var a = 'D:\\Proyectos Sintesis\\Creditos\\SintesisCloud\\Clientes\\' + params.tipo + params.iden;
        params.a = a;

        MethodService("Solicitudes", "Verificarcarpeta", JSON.stringify(params), 'EndCallbackRuta');
    }
}
...
function EndCallbackRuta(param, answer) {
    if (!answer.Error) {
        ...
        EnabledFinish(true);
        MethodUploads("Solicitudes", "SolicitudesSave", data, JSON.stringify(params), "EndCallbackSave");
    }
    else {
    ...
    if (window.gridcotizaciones === undefined) {
        window.gridcotizaciones = $("#tblsolicitudes").bootgrid({
            ...
            window.gridcotizaciones.find(".command-edit").on("click", function (e) {
                ...
                params.id = id;
                newCotizacion();
                MethodService("Solicitudes", "SolicitudesGet", JSON.stringify(params), 'EndCallbackGet');
            });
        });
    }
    else
...
function EndCallbackGet(params, answer) {
    if (!answer.Error) {
        ...
        $('#ds_cliente').val(data.cotizacion);
...
function EndCallbackSave(params, answer) {
    if (!answer.Error) {
        if ($('#id_persona').val() != 0) {
            ...
            parametros.id = answer.Row.clavesolicitud;
...
/* envia los datos que carga Json a "conversor" para convertir la ruta traida de la base de datos en una imagen */
function CrearAgregado(Json) {
    MethodService("Solicitudes", "Conversor", JSON.stringify(Json), 'EndCallbackImgtobase64');  
}

/*crea un recuadro con la informacion basica tanto de el deudor
 como de el/los codeudor/es de una solicitud*/
function EndCallbackImgtobase64(Js, answer) {
    if (!answer.Error) {
        var Json = JSON.parse(Js);
        answer = 'data:image/png;base64,' + answer
        ...
        var a = $('<a class="clearfix" title="Detalle del ' + ((Json.tipo == 'CL') ? "SOLICITANTE" : "CODEUDOR") + '" href="#"/>').attr('data-id', Json.id_solicitud);
        var html = '<div class="col-lg-4 col-md-4 col-sm-3 roundImgParent"><img class="roundImg" src="' + ((Json.urlimg != "" && Json.urlimg != undefined) ? answer : "../Images/defaultperson.jpg") + '"></div>' +
            '<div class="col-lg-8"><p><strong>' + ((Json.tipo == 'CL') ? "SOLICITANTE" : "CODEUDOR") + '</strong><br>' + Json.pnombre + ' ' + Json.snombre + ' ' + Json.papellido + ' ' + Json.sapellido + '<br>' + Json.correo + '<br>' + Json.telefono + ' - ' + Json.celular + '</p></div>';
...
function EndCallbackGetPersona(params, answer) {
    if (!answer.Error) {
        ...
        body.empty();
        $.each(answer.Table, function (i, e) {
            tr = $('<tr/>').attr({ 'data-id': e.id, 'data-nombre': e.nombre, 'data-direccion': e.direccion, 'data-telefono': e.telefono, 'data-tipo': e.id_tipo, 'data-ref': e.tipo, 'data-paren': e.id_parentezco });
            a = $('<a/>').html('<span class="fa fa-2x fa-trash-o text-danger iconfa"></span>').click(function () {
                $(this).closest('tr').remove();
            })
            td = $('<td class="text-center"/>').append(a);
...
function SetDataPerson(dara, TP) {
    ...
    $('#id_persona').val(dara.id_persona);
    ...
    if (dara.urlimg == "" || dara.urlimg == null) {
        ...
        MethodService("Solicitudes", "Conversor", JSON.stringify(dara), 'EndCallbackConverImgtobase64');
...
/* al concvertir la ruta en imagen se le atribuye a el campo de la foto */
function EndCallbackConverImgtobase64(params, answer) {
    if (!answer.Error) {
        $('.dropify-render').append('<img src=" data:image/png;base64,' + answer + '" style="max-heigth:205px">');
        fotoget = 'data:image/png;base64,' + answer;
    }
    else {
        toastr.error(answer.Message, 'Sintesis Creditos');
    }
...
/* bloque los input, select y los botones que se deban bloquear */
function BlockForm() {
    var div = $('#form');
...
$('#iden').blur(function () {
    var value = $(this).val();
    //$('#identificacion').val(calcularDigitoVerificacion(value));
...
/* para abrir el modal para la huella */
//$('#btnCaphue').click(function () {
//    $('#ModalHuella').modal("show");
...
//            console.log(2)
//            //onSamplesAcquired este evento se lanza cuando alguien coloca el dedo sobre el lector
...
//                console.log("Aquisición correcta.");
//            }, function (error) {
...
//        var localStorage = Fingerprint.b64UrlTo64(decodedData.Data);
//        console.log(5)
